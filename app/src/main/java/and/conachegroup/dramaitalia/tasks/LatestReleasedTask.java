...
public class LatestReleasedTask extends AsyncTask<Object, Void, List<LatestReleased>>
    ...
    private WeakReference<LatestReleasedFragment> mLatestReleasedFragment;
    private WeakReference<RelativeLayout> mLoading;
    private WeakReference<RelativeLayout> mError;

    public LatestReleasedTask(LatestReleasedFragment latestReleasedFragment,
                              RelativeLayout loading, RelativeLayout error) {
        mLatestReleasedFragment = new WeakReference<>(latestReleasedFragment);
    ...
    protected void onPostExecute(List<LatestReleased> result) {
        if (result != null && !result.isEmpty()) {
            ...
            mLatestReleasedFragment.get().setupListView(result);
        } else {
